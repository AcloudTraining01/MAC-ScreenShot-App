# SnapForge — Project Context for Gemini / Antigravity

> **Sibling files:** `CLAUDE.md` and `AGENTS.md` (Codex / GPT) carry the same information
> for other tools. **If you change project facts here, update all three** — they are
...
You may be running as Gemini, but the product's AI features are built on
`@anthropic-ai/sdk` with the model `claude-opus-5`. This was an explicit product decision
by the owner, not an accident or a placeholder. Do not "helpfully" port note generation,
chat, or flashcards to the Gemini API, and do not add a Google AI dependency.

The one place Google *is* used is **Google Drive**, for offloading old recordings
(Phase 7). That is storage, not inference.
